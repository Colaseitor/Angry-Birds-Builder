...
func _process(delta):
	if Input.is_action_pressed("Left_Mouse"):
		if state == BirdState.Shot:
			...
			var egg = load("res://Birds/ABClassic/matilda_egg.tscn").instantiate()
			egg.position = global_position + Vector2(0, 70)
...
func unselected():
	if randi_range(1,10) == 10:
		$"90_Sfx-BirdMiscA9".play()

func ready_sound():
	if randi_range(1,10) == 10:
		$"114BirdWhiteReady1".play()
