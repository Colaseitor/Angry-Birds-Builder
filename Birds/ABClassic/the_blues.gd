...
func _process(delta):
	if Input.is_action_pressed("Left_Mouse"):
		if state == BirdState.Shot:
			state = BirdState.Used
			$Special.play()
			$CPUParticles2D2.global_position = position + get_parent().position
			$CPUParticles2D2.emitting = true
			var jay = load("res://Birds/ABClassic/TheBlues.tscn").instantiate()
			var jake = load("res://Birds/ABClassic/TheBlues.tscn").instantiate()
			jay.original = false
			jake.original = false
			jay.position = position + Vector2(-linear_velocity.normalized().y,linear_velocity.normalized().x) * 20
			jake.position = position + Vector2(linear_velocity.normalized().y,-linear_velocity.normalized().x) * 20
			jay.linear_velocity.x = linear_velocity.x * cos(0.17) - linear_velocity.y * sin(0.17)
			jay.linear_velocity.y = linear_velocity.x * sin(0.17) + linear_velocity.y * cos(0.17)
			jake.linear_velocity.x = linear_velocity.x * cos(-0.17) - linear_velocity.y * sin(-0.17)
			jake.linear_velocity.y = linear_velocity.x * sin(-0.17) + linear_velocity.y * cos(-0.17)
			#var max_offset = Vector2((linear_velocity.x + linear_velocity.y) / linear_velocity.x, (linear_velocity.x + linear_velocity.y) / linear_velocity.y)
			#jay.linear_velocity = linear_velocity + (linear_velocity.normalized() / abs(linear_velocity.normalized()) * max_offset - max_offset * linear_velocity.normalized()) * 70
			#jake.linear_velocity = linear_velocity - (linear_velocity.normalized() / abs(linear_velocity.normalized()) * max_offset - max_offset * linear_velocity.normalized()) * 70
			get_parent().add_child(jay)
			get_parent().add_child(jake)
...
func _on_scream_timer_timeout() -> void:
	$BlinkTimer/ScreamTimer.wait_time = randf_range(1,15)
	$Sprite2D.frame = 2
	var pedosdematga = randi_range(1,13)
	if pedosdematga == 1:
	...
	elif pedosdematga == 12:
		$"117BirdBlueTalk1".play()
	else:
...
func unselected():
	if randi_range(1,10) == 10:
		$"116BirdBlueUnselected1".play()

func ready_sound():
	if randi_range(1,10) == 10:
		$"118BirdBlueReady1".play()
